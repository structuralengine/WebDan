...
export class SetDesignForceService {
  ...
  // 断面力手入力情報から断面力一覧を取得
  private getDesignForceFromManualInput(
    calcTarget: string[],
    pickupNo: number
  ): any[] {
    // 部材グループ・照査する着目点を取得
    ...
    for (const groupe of result) {
      for (const member of groupe) {
        ...
        for (const position of member.positions) {
          ...
          const designForce = this.getSectionForce(calcTarget, n, {
            Mdmax: targetForce,
            Mdmin: targetForce,
            Vdmax: targetForce,
            Vdmin: targetForce,
  ...
  // ピックアップデータから断面力一覧を取得
  private getDesignForceFromPickUpData(
    calcTarget: string[],
    pickupNo: number
  ): any[] {
    const target = calcTarget[0];
    ...
    for (const groupe of result) {
      for (const member of groupe) {
        const targetMember = targetForce.filter(
          (value) => value.m_no === member.m_no
        );
        if (targetMember === undefined) {
          return new Array(); // 存在しない要素番号がある
        }
        ...
        for (const position of member.positions) {
          const targetPosition = targetMember.find(
            (value) => value.index === position.index
          );
  ...
  public AlignMultipleLists(...DesignForceListList: any[]) {
    ...
    for (let i = 1; i < DesignForceListList.length; i++) {
      // 全ての着目点情報を収集しておく
      const targetPositions = this.getforceArray(DesignForceListList[i]);
      ...
      for (let ig = 0; ig < baseDesignForceList.length; ig++) {
        ...
        for (let im = 0; im < groupe.length; im++) {
          ...
          for (let ip = 0; ip < member.positions.length; ip++) {
            ...
            const pos = targetDesignForceList[ig][im].positions[ip];

            // 断面力情報をコピー
            for (let i = 0; i < position.designForce.length; i++) {
              const d = position.designForce[i];
              const f = targetPositions.find(
                (t) =>
                  t.index === position.index &&
                  t.side === d.side &&
                  t.target === d.target
              );
  ...
  public getforceArray(DesignForceList: any[]): any {
    const result = [];
